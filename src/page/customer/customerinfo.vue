<template>
  <detail-content :ax="m">
    <div slot="detail" slot-scope="item" class="content">
      ...
      <router-link :to="'/customeraudited/' + $route.params.id" tag="div" class="title">
        <div class="mm">
          <p class="user"><i class="fa fa-user" aria-hidden="true"></i>{{item.detail.detail.body.username}}</p>
          <p class="dsc">{{item.detail.detail.body.parent_customer}}</p>
        </div>
        <div class="tag">
      ...
      <div class="opt">
        <div class="list" @click="getStatus">
          <div class="name" ref="status">{{item.detail.detail.body.status}}</div>
          <div class="tag"><x-icon type="ios-arrow-forward" size="16" fill="white"></x-icon></div>
        </div>
        <router-link :to="'/task/' + $route.params.id + '/customer'" tag="div" class="list">
      ...
      <div class="section top">
        <router-link :to="{}" tag="h2">
          <i class="fa fa-chevron-circle-right" aria-hidden="true"></i>商机
        </router-link>
        <group :gutter="0">
          <div class="nodata">
            暂无商机记录
    ...
    <div class="caidan" slot="toolbar" slot-scope="item" v-if="item.detail">
      <router-link :to="'/writecustomer/' + $route.params.id" tag="div" class="item">
        <span class="icon"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span>
        <span class="text">写跟进</span>
      </router-link>
      <div class="item">
        <a :href="'tel:' + item.detail.detail.body.telphone">
